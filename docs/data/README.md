...
| `2026-08-20-m2max-32gb-0018-session.jsonl` | 13 requests | Every request a real `claude -p` session made on `config/agent.env`, accounted from the server's own log by `cmd/prefixlog`: prompt, ingested, reused, and how the slot was chosen. The prompt is derived rather than printed, so the instrument was held against the eviction file's rows first, where the endpoint's own account agrees on all 15 requests. Read-only tools over a copy of this repo, one question, and no calls beside the conversation at all — `CLAUDE_CODE_DISABLE_TERMINAL_TITLE` in the harness env suppresses them. |
