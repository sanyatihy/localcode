...
| `2026-08-20-m2max-32gb-0017-screen.jsonl` | 11 screens | Both speculative-decoding candidates against 0014's desktop verdict at 32k and 49k, plus the baseline they are read against, each judged from a load rather than a full ladder cell. `smoke` is the row that decides: a server can answer `/health` with 200 and still fail every Metal command buffer out of memory, so a config that cannot generate one token is `admissible: false` whatever its memory reads. Rows with `smoke_prompt_tokens: 0` are load-only and the field is absent on the first six, which predate it; the `-filled` rows send a prompt sized to 90% of the context, which is the only way a runtime that allocates KV per request is judged at its profile at all. Every candidate row failed with `kIOGPUCommandBufferCallbackErrorOutOfMemory`, both with a quantised KV cache and without. `condition` is `unattended` throughout: the desktop verdict is `not_applicable` and the compositor series behind each row was kept. |
